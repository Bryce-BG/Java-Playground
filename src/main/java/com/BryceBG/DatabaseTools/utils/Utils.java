...
public class Utils {
	...
	private static final String[] REQUIRED_CONFIG_KEYS = { "app.dbhost", "app.dbport", "app.dbname", "app.dbpass",
			"app.dbuser" };
	...
	/**
	 * Helper function to searcha PropertiesConfiguration and ensure it contains
	 * required keys for our app to run
	 * 
	 * @param config The currently loaded configuration.
	 * @return True if there are missing critical keys, False if all required keys
	 *         were included.
	 */
	 ...
	 * 
	 * @return 0.0.5 if not compiled to a jar. Otherwise the current version number
	...
	private static String getWindowsConfigDir() {
		return System.getenv("LOCALAPPDATA") + File.separator + GlobalConstants.APP_NAME;
	}
	...
	private static String getUnixConfigDir() {
		return System.getProperty("user.home") + File.separator + ".config" + File.separator + GlobalConstants.APP_NAME;
	}
	...
	private static String getMacOSConfigDir() {
		return System.getProperty("user.home") + File.separator + "Library" + File.separator + "Application Support"
				+ File.separator + GlobalConstants.APP_NAME;
	}

	/**
	 * DEBUG function I am using to verify resources are correct (remove in final
	 * app)
	 * 
	 * @param resourceName the resource embedded in our jar to lookup
	 */
...
//    	InputStream inputStream =  Utils.class.getResourceAsStream(resourceName); //WORKS but needs "/" included in front of name of resource
		...
		try (Scanner s = new Scanner(inputStream).useDelimiter("\\A")) {
			...
			System.out.println("Crashed while trying to get resource\n" + e.getMessage());
		}
		return result;
...
//        builder.setStatusLevel(Level.DEBUG);//status logger is events that occur IN the actual logging system (so we don't want this to be low)
		...
		if (getConfigBoolean("log.save", false)) { // only add file logger appender if config file indicates we should
													// be saving a more permanent log
