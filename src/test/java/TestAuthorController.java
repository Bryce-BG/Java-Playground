...
public class TestAuthorController {
	...
	public void testUpdateAuthorBio() {
		...
		//Test 2: preform invalid updates (fails but doesn't crash)
				...
				assertFalse(AuthorController.updateAuthorBio("admin", "Password1", "", "Joyce", author_bio).getValue0().booleanValue());
				
				//Test 2.e: invalid authentication username
				assertFalse(AuthorController.updateAuthorBio("admn", "Password1", null, "Joyce", author_bio).getValue0().booleanValue());
				//Test 2.f: invalid password
				assertFalse(AuthorController.updateAuthorBio("admin", "Password", null, "Joyce", author_bio).getValue0().booleanValue());
				//Test 2.g: user preforming update who isn't the verified_owner or an admin
				assertFalse(AuthorController.updateAuthorBio("JamesJoyce", "Password1", "James", "Joyce", author_bio).getValue0().booleanValue());
	...
	public void testVerifyAuthorAccount() {
		//Test 1: valid update to user verified_id
				assertTrue(AuthorController.verifyAuthorAccount("admin", "Password1", "James", "Joyce", "JamesJoyce").getValue0().booleanValue());
				
				
				//Test 2: preform invalid updates (fails but doesn't crash)
						//Test 2.a: empty username for validated_user update (should fail)
				assertFalse(AuthorController.verifyAuthorAccount("admin", "Password1", "James", "Joyce", "").getValue0().booleanValue());

						//Test 2.b: misspelled identifier for the author (james)
				assertFalse(AuthorController.verifyAuthorAccount("admin", "Password1", "jaes", "Joyce", "JamesJoyce").getValue0().booleanValue());
						//Test 2.c: null author first name field
				assertFalse(AuthorController.verifyAuthorAccount("admin", "Password1", null, "Joyce", "JamesJoyce").getValue0().booleanValue());
						//Test 2.d: empty first author first name
				assertFalse(AuthorController.verifyAuthorAccount("admin", "Password1", "", "Joyce", "JamesJoyce").getValue0().booleanValue());
						
						//Test 2.e: invalid authentication username
				assertFalse(AuthorController.verifyAuthorAccount("admn", "Password1", "James", "Joyce", "JamesJoyce").getValue0().booleanValue());
						//Test 2.f: invalid password
				assertFalse(AuthorController.verifyAuthorAccount("admin", "Password", "James", "Joyce", "JamesJoyce").getValue0().booleanValue());
						//Test 2.g: user preforming update isn't an admin
				assertFalse(AuthorController.verifyAuthorAccount("JamesJoyce", "Password1", "James", "Joyce", "JamesJoyce").getValue0().booleanValue());
	}
}
